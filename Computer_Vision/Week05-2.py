...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32*32*3, 784)  # 3072차원(32x32x3) -> 256차원으로 축소
        self.fc2 = nn.Linear(784, 256)
        self.fc3 = nn.Linear(256, 100)  # 출력값의 차원은 판별할 클래스 수인 10으로 설정 (MNIST는 손글씨 숫자 10종 판별 문제)
...
test_batch_size = 2000  # 테스트 배치 크기 (학습 과정을 제외하므로 더 큰 배치 사용 가능)
...
train_loader = torch.utils.data.DataLoader(
  datasets.CIFAR100(os.path.join(workspace_path, 'data'), train=True, download=True, transform=transform), 
...
test_loader = torch.utils.data.DataLoader(
        datasets.CIFAR100(os.path.join(workspace_path, 'data'), train=False, download=True, transform=transform), 
...
CIFAR-100 데이터셋에 대한 인공신경망 모델을 학습/추론하는 코드+출력결과
- torchvision: 데이터셋, 데이터증대 함수 제공
